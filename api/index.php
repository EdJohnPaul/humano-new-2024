...
$app->get("/test",function(){
    // $requid = "039E1F7A-0ED4-F1C0-617A-DD465F148B93";
    // read_cash_advance_loan_request_requestuid_reinvent($requid);
    // get_loan_type($loanName);
    // $uid = xguid();
    // echo $uid;
    // $refNo = generateRefNo();
    // echo "<br>";
    // echo $refNo;
    // echo "<br>";
    // read_all_company_loan();
    // $emp_uid = "1833AD97-1A5D-BCAB-DB68-58C7EC616EA4";
    // get_all_company_loan_request_by_emp_uid($emp_uid);
    // read_emp_company_loans($emp_uid);
    $applicationNo = 2023080337408964;
    $sql = ORM::forTable("emp_loans_payments")->where('application_no','2023080337408964')->findMany();
    foreach($sql as $result)
    {
        $data[] = array("empUid"=>$result->emp_uid,"loanUid"=>$result->loan_uid,"applicationNo"=>$result->application_no,"referenceNo"=>$result->reference,"amountPaid"=>$result->amount_paid,"balance"=>$result->balance,"datePosted"=>$result->date_created);
    }
    echo jsonify($data);
});

$app->get("/deminimis", function() {
    $emp_uid = "F10E74AA-16C1-9E3A-226D-8D151C8B1B2C";

    $reg = read_payroll_registry_by_uid('79A3242F-4B23-65D4-71CB-05FEB1D1C5FF');
    $cut_off = intval($reg['period']); 
    echo $cut_off . "</br>";

    $period = getEmployeeAllowanceByEmpUid($emp_uid);
...
$app->get("/resize/image/", function() {    
    ...
    $img = resize_image(__DIR__ . '/files/51367.jpg', 200, 200);
    echo "<img src='".$img."'/>";
});

$app->get("/password/generator/:var", function($var) {
    echo md5($var);
});
...
$app->get("/sample", function() {
    ...
    */
    //echo var_dump($query);
    $uid = "C659EAB5-5AE2-84EF-9F66-59320A98A3AC";
    $results = getEmploymentStatusByEmpUidPages2($uid);
    //echo "</br>";
    //echo $result->date_hired;
...
$app->post("/process/payroll/:var", function($var) {
	...
	if(count($param) === 2) {
		...
		foreach($results as $result) {
			...
			$overtime = compute_overtime_holiday($hourly, $ot, $rdot, $shot, $shrdot, $rhot, $rhrdot, $rd, $sh, $shrd, $rh, $rhrd, $nd);
			# Overtime #
            ...
            $cash_advance_loan_amount = get_employee_loans_payroll_amounts("cash",$emp_uid, $trans_date);
            $emp_loans = get_employee_loans_and_loans_by_loan_type_and_emp_uid_once("cash", $emp_uid);
            if($emp_loans) {
                $uuid = $emp_loans->uid;
                create_emp_loans_payments($uuid, $cash_advance_loan_amount);
            }
            ...
            $isValid = validateEmployeeAllowanceByEmpUid($emp_uid);
            if($isValid!=0) {
                $pperiod = getEmployeeAllowanceByEmpUid($emp_uid);            
                $period_type = $pperiod->period;
			...
			if($actionStatus!="POSTED") {
				...
				create_payroll_summary($uid, $emp_uid, $period, $salary, $daysPresent, $daysAbsent, $grossPay, $deminimis, $ecola, $overtime, $totalTardiness, $sssEmployee, $philEmployee, $pagibig, $sss_loan_amount,$hdmf_loan_amount, $company_loan_amount, $taxValue, $adj_add, $adj_less, $netPay, $cash_advance_loan_amount);
			}
		}
...
$app->get("/getemptaxbracket/:var", function($salary) {
    $param = explode(".", $salary);
    $response = array();

    $uid = "CF29955E-CA94-0628-85BD-3164675393B0";
    $row = null;
    $based = null;
    $over = null;
    $percent = null;
    ...
    if(count($param)===1) {
        $query = ORM::forTable("new_tax_table")->findMany();
        foreach($query as $results) {
            $x = $results->x; $y = $results->y; $xuid = "CF29955E-CA94-0628-85BD-3164675393B9";
            if($salary>$x && $salary<=$y) {
                $uid = $results->payroll_uid;
                $row = $results->row;
                $based = $results->based;
                $over = $results->over;
    ...
    $salary = (float) $salary; //echo $salary . "</br>";
    $over = (float) $over;
    $subs = $salary - $over; echo "Over: " . $subs . "</br>";
    $percent = $subs * ($percent / 100 ); echo "Percent: " . $percent . "</br>";
    $tax = $based + $percent; echo "Tax:" . $tax;
});

/* Ed Update July 17, 2023 START */
...
$app->post("/cash/advance/loan/request/:var",function($var){
...
$app->get("/company/loan/request/view/selected/:var",function($var){
    ...
    if(count($param)===2)
    {
        ...
        $info = array(
            ...
            "releaseType"=>$result['requestInfo'][14]['release_type'],
            "bank"=>$result['requestInfo'][15]['bank'],
            "accountNo"=>$result['requestInfo'][16]['account_no'],
            "voucherNo"=>$result['requestInfo'][17]['voucher_no'],
            "voucherDate"=>$result['requestInfo'][18]['voucher_date'],
            "checkNo"=>$result['requestInfo'][19]['check_no'],
            "checkDate"=>$result['requestInfo'][20]['check_date']
        );
    }
    $response = array($info);
    echo jsonify($response);
...
$app->post("/cash/advance/loan/approve/:var",function($var){
    ...
    $application_no = $_POST['applicationNo'];
    if(count($param) === 2)
    {
        create_emp_loans_new_trial($empUid,$amortization,$terms,$amount,$remarks,$loan_type,$application_no);
...
$app->post("/cash/advance/release/:var",function($var){
    $param = explode(".",$var);
    $verified = 1;
    $error = 0;
    $response = array();
    ...
    if(count($param)===2)
    {
        if($releaseType === "cash")
        {
            $voucher_no = $_POST['voucherNo'];
            $voucher_date = $_POST['voucherDate'];
            $keys = array("release_type","bank","account_no","voucher_no","voucher_date");
            $values = array($releaseType,$bank,$account_no,$voucher_no,$voucher_date);
            for($i=0;$i<count($keys);$i++)
            {
                update_cash_advance_loan_request_entry_for_cash($requestUid,$keys[$i],$values[$i]);
            }
            $error = 0;
            $verified = 1;
        }
        else if($releaseType === "check")
        {
            $check_no = $_POST['checkNo'];
            $check_date = $_POST['checkDate'];
            $keys = array("release_type","bank","account_no","check_no","check_date");
            $values = array($releaseType,$bank,$account_no,$check_no,$check_date);
            for($i=0;$i<count($keys);$i++)
            {
                update_cash_advance_loan_request_entry_for_cash($requestUid,$keys[$i],$values[$i]);
            }
...
/* Ed Update July 17, 2023 END */

$app->post("/company/loan/new/offer/:var",function($var){
    $param = explode(".",$var);
    $loanAmount = $_POST['loanAmount'];
    $loanTerm = $_POST['loanTerm'];
    $amortization = $_POST['amortization'];
    ...
    $success = 1;
    $error = 0;
    $value = array($loanAmount,$loanTerm,$amortization,$interest,$allowed,$payment,$status);

    if(count($param)===2)
    {
        create_company_loan_entry($value);
        $success = 1;
        $error = 0;
    }
    else
...
$app->get("/view/company/loan/setup/:var",function($var){
    $param = explode(".",$var);
    $data = array();
    if(count($param)===2)
    {
        $getCompanyLoans = read_all_company_loan();
        foreach($getCompanyLoans as $result)
        {
            $data[] = array(
                "companyLoanUid"=>$result["uid"],
                "loan_amount"=>$result['companyLoanInfo'][0]['loan_amount'],
                "term"=>$result['companyLoanInfo'][1]['term'],
                "amortization"=>$result['companyLoanInfo'][2]['amortization'],
                "interest"=>$result['companyLoanInfo'][3]['interest'],
                "allowable"=>$result['companyLoanInfo'][4]['allowable'],
                "payment"=>$result['companyLoanInfo'][5]['payment'],
                "status"=>$result['companyLoanInfo'][6]['status']
            );
        }
    }
    echo jsonify($data);
...
$app->get("/edit/company/loan/:var",function($var){
    $param = explode(".",$var);
    $uid = $param[1];
    $data = array();
    if(count($param)===2)
    {
        $getCompanyLoan[] = read_company_loan_entry($uid);
        foreach($getCompanyLoan as $result)
        {
            $data = array(
                "companyLoanUid"=>$uid,
                "loan_amount"=>$result[0]['loan_amount'],
                "term"=>$result[1]['term'],
                "amortization"=>$result[2]['amortization'],
                "interest"=>$result[3]['interest'],
                "allowable"=>$result[4]['allowable'],
                "payment"=>$result[5]['payment'],
                "status"=>$result[6]['status']
            );
        }
    }
    echo jsonify($data);
...
$app->get("/read/enabled/company/loan/:var",function($var){
    $param = explode(".",$var);
    $uid = $param[1];
    $data = array();
    if(count($param)===2)
    {
        $getCompanyLoans = read_enabled_company_loan_entry($uid);
        foreach($getCompanyLoans as $result)
        {
            $data[] = array(
                "companyLoanUid"=>$result["uid"],
...
$app->post("/save/company/loan/setup/changes/:var",function($var){
    $param = explode(".",$var);
    $uid = $_POST['loanuid'];
    ...
    $payment = $_POST['payment'];
    $status = $_POST['status'];
    $values = array($loan_amount,$term,$amortization,$interest,$allowable,$payment,$status);
    $verified = 1;
    $error = 0;
    if(count($param)==2)
    {
        update_company_loan_entry($uid,$values);
        update_company_loan_status($uid,$status);
    }
    else
    {
        $verified = 0;
...
$app->post("/request/company/loan/:var",function($var){
    $param = explode(".",$var);
    $req_uid = xguid();
    $error = 1;
    $verified = 0;
    ...
    if(count($param)==2)
    {
        //create company loan request function
        create_company_loan_request($req_uid,$emp_uid,$loan_uid);
        create_company_loan_request_entry($req_uid,$emp_uid,$loan_uid,$application_no,$empEmail,$loanAmount,$loanTerm,$amortization,$interest,$payment);
        mail_company_loan_request_to_admin($empEmail,$empName,$loanAmount);
        $verified = 1;
        $error = 0;
    }
    else
...
$app->get("/read/all/own/request/:var",function($var){
    $param = explode(".",$var);
    $emp_uid = $param[1];
    $data = array();
    if(count($param)===2)
    {
        $data = read_emp_company_loans($emp_uid);
    }
    else
    {
        $data[] = array();
...
$app->get("/read/all/company/loan/requests/:var",function($var){
    $param = explode(".",$var);
    if(count($param)==2)
    {
        $data = read_all_company_loan_request();
    }
    else
    {
        $data = array();
...
$app->get("/read/company/loan/request/status/:var",function($var){
    $param = explode(".",$var);
    $status = $param[0];
    if(count($param) === 3)
    {
        $data = read_all_company_loan_request_by_status($status);
    }
    echo jsonify($data);
});
$app->get("/read/company/loan/detailed/view/:var",function($var){
    $param = explode(".",$var);
    $requid = $param[2];
    if(count($param) === 3)
    {
        $data = read_company_loan_request_entry($requid);
    }
    echo jsonify($data);
});
$app->run();
