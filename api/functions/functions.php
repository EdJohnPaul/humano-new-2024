...
function getEmploymentStatusByEmpUidPages2($uid){
    $response = array();
    $query = ORM::forTable("emp_type")->tableAlias("t1")->innerJoin("hris_employment_status", array("t1.employment_status_uid", "=", "t2.employment_status_uid"), "t2")->where("t1.emp_uid", $uid)->where("t1.status", 1)->findMany();
    //return $query;
...
function getOldTimeIn($id, $date){
    $query = ORM::forTable("time_log")
    ->rawQuery("SELECT date_created, emp_uid, session, shift_uid FROM time_log WHERE emp_uid = :id AND type='0' AND status='1' AND date(date_created) = :dates ORDER BY id ASC", array("id" => $id, "dates" => $date))
    ->findOne();
...
function getAllowanceByEmpUidPeriod($emp_uid, $period) {
    $query = ORM::forTable("allowance")->where("emp_uid", $emp_uid)->where("period", $period)->where("status", 1)->findOne();
    return $query;
}

function getEmployeeAllowanceByEmpUid($emp_uid) {
    $query = ORM::forTable("allowance")->where("emp_uid", $emp_uid)->where("status", 1)->findOne();
    return $query;
}

function validateEmployeeAllowanceByEmpUid($emp_uid) {
    $query = ORM::forTable("allowance")->where("emp_uid", $emp_uid)->where("status", 1)->count();
    return $query;
}
